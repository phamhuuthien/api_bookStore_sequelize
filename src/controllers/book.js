...
import {title,available,image,category_code,price,bid,bids,filename,stars} from '../helpers/joi_schema'
import joi from 'joi'


const cloudinary = require('cloudinary').v2
// gửi params lấy query 
// gửi body lấy body 
...
export const createNewBook = async (req,res) => {
    try {
        ...
        const { error } = joi.object({title,image,category_code,price,available,stars}).validate({...req.body,image : fileData.path})
        if(error){      
